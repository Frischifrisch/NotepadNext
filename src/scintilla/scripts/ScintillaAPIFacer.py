...
def ActualTypeName(type, identifier=None):
	return typeAliases[type] if type in typeAliases else type
...
def JoinTypeAndIdentifier(type, identifier):
	...
	return type + identifier if type.endswith("*") else f"{type} {identifier}"
...
def ParametersArgsCallname(v):
	...
	if param1Type:
		...
		if param1Type.endswith("*"):
			castName = f"reinterpret_cast<uintptr_t>({param1Name})"
		elif param1Type not in basicTypes:
			castName = f"static_cast<uintptr_t>({param1Name})"
	...
	if param2Type:
		...
		if param2Type.endswith("*"):
			callName = "CallString" if param2Type == "const char *" else "CallPointer"
		elif param2Type not in basicTypes:
			castName = f"static_cast<intptr_t>({param2Name})"
...
def ParametersExceptLast(parameters):
	return parameters[:parameters.rfind(",")] if "," in parameters else ""
...
def HEnumerations(f):
	...
	out.extend(("", "enum class Notification {"))
	for name in f.order:
		v = f.features[name]
		if v["Category"] != "Deprecated":
			...
			if v["FeatureType"] in ["val"]:
				...
				if not hasPrefix:
					...
					if name == "INVALID_POSITION":
						type = "Position"
					out.append(f"constexpr {type} {Face.PascalCase(name)} = " + v["Value"] + ";")
	return out
	...
	for name in f.order:
		...
		if v["Category"] != "Deprecated":
			if v["FeatureType"] in ["fun", "get", "set"]:
				...
				out.append(
					f'{JoinTypeAndIdentifier(retType, f"ScintillaCall::{name}")}({parameters})'
					+ " {"
				)
				retCast = ""
				retCastEnd = ""
				if retType not in basicTypes or retType in ["int", "Colour", "ColourAlpha"]:
					if IsEnumeration(retType):
						retType = namespace + retType
					retCast = f"static_cast<{retType}>("
					retCastEnd = ")"
				elif retType in ["void *"]:
					retCast = f"reinterpret_cast<{retType}>("
					retCastEnd = ")"
				out.extend(
					(
						"\t"
				...
				if v["Param2Type"] == "stringresult":
					...
					out.extend(
						(
							JoinTypeAndIdentifier("std::string", f"ScintillaCall::{name}")
							+ "("
							+ paramList
							+ ") {",
							"\treturn CallReturnString(" + msgName + argList + ");",
							"}",
							"",
						)
					)
